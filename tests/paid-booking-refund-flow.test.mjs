...
test("refund preparation blocks paid-out seller settlements and separately resolved deposits", () => {
  assert.match(migration, /s\.status='settled'/);
  assert.match(migration, /deposit_status,'not_charged'\) not in \('held','not_charged'\)/);
});

test("refund finalizer cancels order and seller settlement only after external refund", () => {
