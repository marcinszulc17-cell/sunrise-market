// ad-buy — zakup reklamy przez sprzedawcę. Płatność portfelem Sunrise Pay. Tworzy kampanię.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { rate_code, offer_id, budget } = await req.json();
    if (!rate_code || !offer_id) return json({ error: "rate_code i offer_id wymagane" }, 400);

    const { data: rate } = await sb.from("ad_rates").select("*").eq("code", rate_code).eq("active", true).maybeSingle();
    if (!rate) return json({ error: "Nieznana stawka" }, 400);
    const { data: offer } = await sb.from("offers").select("id,title,seller_id").eq("id", offer_id).maybeSingle();
    if (!offer) return json({ error: "Nie znaleziono produktu" }, 404);

    const amount = rate.model === "cpc" ? Math.max(20, Number(budget ?? 20)) : Number(rate.price);
    const amountGrosz = Math.round(amount * 100);
    const key = await uuidv5(`market:ad:${user.id}:${offer_id}:${rate_code}:${Date.now()}`);
    const charge = await pay("pay-charge", { user_ref: user.email, amount_grosz: amountGrosz, order_ref: `ad-${offer_id}`, idempotency_key: key });
    if (charge.status === 402 || (charge.data && charge.data.ok === false && charge.data.error === "insufficient_funds"))
      return json({ error: "Za mało środków w portfelu Sunrise Pay", need_topup: true, required: amount, balance: Number(charge.data?.balance_grosz ?? 0) / 100 }, 402);
    ...
    const ends = rate.period_days ? new Date(Date.now() + rate.period_days * 86400000).toISOString() : new Date(Date.now() + 30 * 86400000).toISOString();
    const { data: camp } = await sb.from("ad_campaigns").insert({
      seller_id: offer.seller_id, offer_id, rate_code, placement: rate.placement, model: rate.model,
      price_paid: amount, budget: rate.model === "cpc" ? amount : null, status: "active", ends_at: ends,
    }).select("id,placement,ends_at").single();
    return json({ ok: true, campaign: camp, paid: amount, balance: Number(charge.data.balance_grosz ?? 0) / 100 });
