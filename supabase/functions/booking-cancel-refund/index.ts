...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: buyerData, error: buyerError } = await service.auth.admin.getUserById(buyerId);
    ...
    if (!reversal.ok) {
      ...
      await service.rpc("booking_refund_abort", { p_booking: bookingId, p_error: reason });
    ...
    if (row.payment_provider === "sunrise_pay") {
      const idem = await uuidv5(`booking-full-refund:${bookingId}`);
      const credited = await payCredit(buyerEmail, amountGrosz, orderId, idem);
