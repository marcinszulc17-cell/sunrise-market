...
async function payCredit(userRef: string, amountGrosz: number, reason: string, orderRef: string, idem: string) {
  ...
  const response = await fetch(`${PAY_BASE}/pay-credit`, {
    ...
    body: JSON.stringify({ user_ref: userRef, amount_grosz: amountGrosz, reason, order_ref: orderRef, idempotency_key: idem }),
  });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (amountGrosz <= 0) throw new Error("Kaucja ma nieprawidłową kwotę");

    if (action === "refund") {
      if (row.payment_provider === "sunrise_pay") {
        const idem = await uuidv5(`booking-deposit-refund:${bookingId}`);
        await payCredit(String(row.buyer_email), amountGrosz, "Zwrot kaucji Sunrise Market", String(row.order_id), idem);
        ...
        throw new Error("Nieobsługiwana metoda płatności kaucji");
      }

      const { error: finishError } = await service.from("bookings").update({
        deposit_status: "refunded",
        deposit_resolved_at: new Date().toISOString(),
        deposit_retained_gross: 0,
        deposit_resolution_note: note,
        updated_at: new Date().toISOString(),
      }).eq("id", bookingId).eq("deposit_status", "refunding");
      if (finishError) throw finishError;
      return json({ ok: true, action: "refund", deposit_status: "refunded", amount });
    }

    const idem = await uuidv5(`booking-deposit-retain:${bookingId}`);
    await payCredit(String(row.seller_email), amountGrosz, "Zatrzymana kaucja Sunrise Market", String(row.order_id), idem);
    const { error: finishError } = await service.from("bookings").update({
      deposit_status: "retained",
      deposit_resolved_at: new Date().toISOString(),
      deposit_retained_gross: amount,
      deposit_resolution_note: note,
      updated_at: new Date().toISOString(),
    }).eq("id", bookingId).eq("deposit_status", "retaining");
    if (finishError) throw finishError;
    return json({ ok: true, action: "retain", deposit_status: "retained", amount });
  } catch (error) {
    await service.from("bookings").update({
      deposit_status: "failed",
      deposit_resolution_note: String((error as Error).message ?? error).slice(0, 1000),
      updated_at: new Date().toISOString(),
    }).eq("id", bookingId).in("deposit_status", ["refunding", "retaining"]);
