...
async function payCredit(userRef: string, amountGrosz: number, reason: string, orderRef: string, idemName: string) {
  if (!PAY_TOKEN) throw new Error("Brak konfiguracji Sunrise Pay");
  const idem = await uuidv5(idemName);
  ...
  try {
    ...
    if (action === "refund") {
      if (row.payment_provider === "sunrise_pay") {
        await payCredit(String(row.buyer_email), amountGrosz, "Zwrot kaucji Sunrise Market", String(row.order_id), `booking-deposit-refund:${bookingId}`);
