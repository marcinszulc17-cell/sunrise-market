...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (reversed.response.status !== 200 || reversed.data?.ok !== true) {
      ...
      const friendly = reason === "points_already_used"
        ? "Nie można wykonać automatycznego zwrotu, ponieważ część cashbacku lub prowizji z tej rezerwacji została już wykorzystana. Zwrot wymaga rozliczenia przez operatora."
        : "Nie udało się bezpiecznie cofnąć cashbacku i prowizji. Zwrot nie został wykonany.";
    ...
    if (row.payment_provider === "sunrise_pay") {
      externalRef = await refundWallet(buyerEmail, amountGrosz, orderId, bookingId);
      ...
      }, { idempotencyKey: `market-booking-full-refund:${bookingId}` });
