...
async function uuidv5(name: string): Promise<string> {
  ...
  const nsBytes = (ns.replace(/-/g, "").match(/.{2}/g) as string[]).map((hex) => parseInt(hex, 16));
...
async function marketBridge(action: "reverse" | "restore", orderId: string) {
  if (!PAY_TOKEN) throw new Error("Brak konfiguracji MySunrise");
...
async function payCredit(userRef: string, amountGrosz: number, orderId: string, bookingId: string) {
  ...
  const idempotencyKey = await uuidv5(`market:booking-refund:${bookingId}`);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const reversal = await marketBridge("reverse", orderId);
    if (reversal.status !== 200 || reversal.data?.ok !== true) {
      ...
      if (reason === "points_already_used") {
        return json({ ok: false, error: "points_already_used", message: "Cashback lub prowizja z tej rezerwacji zostały już wykorzystane. Zwrot wymaga rozliczenia operatora i nie został wykonany." }, 409);
      }
      return json({ ok: false, error: reason, message: "Nie udało się bezpiecznie cofnąć cashbacku/prowizji. Płatność nie została zwrócona." }, 409);
    }
    ...
    if (buyerError || !buyerEmail) throw new Error("Nie znaleziono e-maila kupującego do zwrotu");
    ...
    if (finalizeError || finalized?.ok !== true) {
      await service.from("booking_refunds").update({
        ...
        last_error: finalizeError?.message ?? "finalize_failed",
        updated_at: new Date().toISOString(),
      }).eq("booking_id", bookingId);
      return json({ ok: false, error: "finalize_failed", message: "Płatność została zwrócona i bonusy cofnięte, ale finalizacja statusu wymaga operatora.", booking_id: bookingId, order_id: orderId }, 500);
    }

    return json({ ok: true, booking_id: bookingId, order_id: orderId, amount, payment_provider: row.payment_provider });
  } catch (error) {
    const message = String((error as Error).message ?? error);
    if (bonusesReversed && orderId) {
      const restore = await marketBridge("restore", orderId).catch(() => null);
      const restoreFailed = !restore || restore.status !== 200 || restore.data?.ok !== true;
      await service.from("booking_refunds").update({
        status: "payment_failed",
        last_error: restoreFailed ? `${message}; bonus_restore_failed` : message,
        updated_at: new Date().toISOString(),
      }).eq("booking_id", bookingId);
      if (restoreFailed) {
        return json({ ok: false, error: "payment_failed_restore_failed", message: "Zwrot płatności nie powiódł się, a przywrócenie bonusów wymaga operatora. Rezerwacja nie została anulowana." }, 500);
      }
    }
