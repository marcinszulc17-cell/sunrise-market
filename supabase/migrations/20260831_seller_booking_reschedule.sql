create or replace function market.seller_booking_reschedule(
  p_booking uuid,
...
begin
  ...
  if p_booking is null or p_starts_at is null then
    raise exception 'Wybierz rezerwację i nowy termin';
  end if;
  ...
  if not (v_booking.seller_id = market.current_seller_id() or market.is_operator()) then
    raise exception 'Brak dostępu';
  end if;
  if v_booking.status <> 'confirmed' then
    raise exception 'Termin można zmienić tylko dla potwierdzonej rezerwacji';
  end if;
  ...
  if v_config.offer_id is null then
    raise exception 'Brak konfiguracji bookingu dla tej oferty';
  end if;
  ...
  if v_booking.booking_type = 'appointment' then
    ...
    if v_booking.service_id is not null then
      ...
      where s.id = v_booking.service_id
        and s.offer_id = v_booking.offer_id
        and s.active;
      if not found then
        raise exception 'Usługa przypisana do rezerwacji nie jest już aktywna';
      end if;
    end if;

    if v_booking.resource_id is not null and not exists (
      select 1
      from market.booking_offer_resources r
      where r.offer_id = v_booking.offer_id
        and r.resource_id = v_booking.resource_id
    ) then
      raise exception 'Zasób przypisany do rezerwacji nie jest już dostępny';
    end if;
    ...
    if not exists (
      ...
      raise exception 'Nowy termin nie mieści się w dostępności oferty';
    end if;
  ...
  if v_start < now() + make_interval(hours => v_config.min_notice_hours) then
    raise exception 'Nowy termin jest zbyt bliski';
  end if;
  if v_start > now() + make_interval(days => v_config.max_advance_days) then
    raise exception 'Nowy termin jest zbyt odległy';
  end if;
  if v_end <= v_start then
    raise exception 'Nieprawidłowy zakres rezerwacji';
  end if;
  ...
  if exists (
    ...
    where x.id <> v_booking.id
      and (
        x.offer_id = v_booking.offer_id
        or (v_booking.resource_id is not null and x.resource_id = v_booking.resource_id)
      )
      and (
        x.status = 'confirmed'
      ...
      and tstzrange(
        x.starts_at - make_interval(mins => v_before),
        x.ends_at + make_interval(mins => v_after),
        '[)'
      ) && tstzrange(v_start, v_end, '[)')
  ) then
    raise exception 'Nowy termin koliduje z inną rezerwacją';
  end if;

  if exists (
    select 1
    from market.booking_blocks z
  ...
  values (
    ...
    case
      when v_booking.booking_type = 'daily' then
        coalesce(v_title, 'Rezerwacja') || ': nowy termin ' ||
        to_char(v_start at time zone v_config.timezone, 'DD.MM.YYYY') || ' – ' ||
        to_char(v_end at time zone v_config.timezone, 'DD.MM.YYYY') || '.'
      else
        coalesce(v_title, 'Rezerwacja') || ': nowy termin ' ||
        to_char(v_start at time zone v_config.timezone, 'DD.MM.YYYY HH24:MI') || '.'
    end
  );
