-- Canonical production safety guard between seller payout retry and paid-booking refunds.
-- Mirrors the live migration refund_payout_atomic_claim_canonical.
...
begin
  ...
  perform 1 from market.seller_settlements s where s.order_id=o.id for update;
  ...
  if exists(select 1 from market.seller_settlements s where s.order_id=o.id and s.status='processing') then
    raise exception 'Wypłata sprzedawcy jest właśnie przetwarzana — spróbuj ponownie po zakończeniu rozliczenia';
  end if;
