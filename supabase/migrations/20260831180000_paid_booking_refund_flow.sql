-- Safe refund flow for paid bookings.
-- Paid bookings cannot be cancelled through the plain status action: money and bonuses
-- must be reversed first through booking-refund-action.
...
begin
  ...
  update market.ambassador_commission_outbox set status='reversed',reason='Prowizje cofnięte po zwrocie opłaconej rezerwacji',updated_at=now()
    where order_id=r.order_id and status in ('sent','failed','pending_vat','pending_identity','ready');
