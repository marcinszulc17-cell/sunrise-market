-- Full paid-booking cancellation/refund flow.
-- Keeps seller payout frozen while refund is in progress and blocks plain cancellation of paid bookings.
...
begin
  ...
  on conflict(booking_id) do update
    set status='preparing',amount_gross=excluded.amount_gross,payment_provider=excluded.payment_provider,last_error=null,updated_at=now();
...
begin
  ...
  update market.seller_settlements
    set status=case when available_at is not null then 'scheduled' else 'pending' end,last_error=null,updated_at=now()
