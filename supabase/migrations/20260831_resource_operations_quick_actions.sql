...
as $$
  with seller_resources as (
    ...
    where t.ends_at > now() and coalesce(t.reason,'') like '[STATUS] %'
    order by t.resource_id, t.created_at desc
  ), current_booking as (
    select distinct on (b.resource_id)
      b.resource_id,b.id,o.title,b.booking_type,u.email::text as buyer_email,b.starts_at,b.ends_at
    ...
    select distinct on (b.resource_id)
      b.resource_id,b.id,o.title,b.booking_type,u.email::text as buyer_email,b.ends_at
    from market.bookings b
    join market.offers o on o.id=b.offer_id
    left join auth.users u on u.id=b.buyer_id
    where b.resource_id is not null
    ...
    select distinct on (t.resource_id) t.resource_id,t.reason,t.ends_at
    from market.booking_resource_time_off t
    where t.ends_at > now()
  ...
  select
    r.id,r.name::text,r.kind::text,r.active,
    case
    ...
    cb.id,cb.title::text,cb.booking_type::text,cb.buyer_email,cb.starts_at,cb.ends_at,
    nb.id,nb.title::text,nb.booking_type::text,nb.buyer_email,nb.starts_at,nb.ends_at,
    ob.id,ob.title::text,ob.booking_type::text,ob.buyer_email,ob.ends_at,
    m.reason::text,m.ends_at
