-- purchase_mode is public buyer-facing behavior, not a private identifier.
-- Keep VIN / registration / land-register identifiers hidden while allowing the
-- universal search UI to distinguish sale, appointment and daily rental offers.
...
returns table(
  offer_id uuid,title text,price_gross numeric,category text,category_slug text,seller text,
  score real,rating numeric,reviews integer,image_url text,attributes jsonb
)
language sql
stable
security definer
...
revoke all on function market.search_offers_v2(text,text,numeric,numeric,text,integer,jsonb) from public;
