-- Keep the tracked refund contract aligned with production: the refund edge function
-- receives the buyer email directly from the authenticated seller-only RPC.
...
begin
  ...
  select u.email::text into v_email from auth.users u where u.id=o.buyer_id limit 1;
  if v_email is null then raise exception 'Brak adresu e-mail kupującego'; end if;
  ...
  if r.booking_id is not null and r.status='refunded' then
    return query select v.id,o.id,o.buyer_id,v_email,o.payment_provider,o.stripe_session_id,o.total_gross,coalesce(v.deposit_gross,0),true;
    return;
  end if;
  ...
  return query select v.id,o.id,o.buyer_id,v_email,o.payment_provider,o.stripe_session_id,o.total_gross,coalesce(v.deposit_gross,0),false;
end;
$$;
