-- Automatic seller refunds are only safe before the booked service/rental starts.
...
begin
  ...
  if v.starts_at <= now() then raise exception 'Po rozpoczęciu terminu zwrot wymaga ręcznej obsługi operatora'; end if;
