-- Safe full refund flow for paid bookings.

alter table market.ambassador_commission_outbox
  drop constraint if exists ambassador_commission_outbox_status_check;
alter table market.ambassador_commission_outbox
...
grant all on table market.booking_refunds to service_role;
...
set search_path = ''
as $$
declare v market.bookings%rowtype;
begin
...
begin
  ...
  if r.booking_id is not null and r.status='refunded' then
    return query select v.id,o.id,o.buyer_id,o.payment_provider,o.stripe_session_id,o.total_gross,coalesce(v.deposit_gross,0),true; return;
  end if;
...
begin
  ...
  update market.bookings set status='cancelled',cancelled_at=coalesce(cancelled_at,now()),hold_expires_at=null,
    ...
    deposit_resolution_note=case when coalesce(deposit_gross,0)>0 then 'Zwrot pełnej płatności przy anulowaniu rezerwacji' else deposit_resolution_note end,updated_at=now()
  where id=p_booking;
  ...
  update market.ambassador_commission_outbox set status='reversed',updated_at=now() where order_id=r.order_id and status in ('sent','failed','pending_vat','pending_identity','ready');
...
revoke all on function market.seller_booking_refund_prepare(uuid) from public,anon,authenticated;
