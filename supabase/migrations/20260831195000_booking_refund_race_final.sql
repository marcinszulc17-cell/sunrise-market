-- Final concurrency and audit guards for paid booking refunds.

alter table market.booking_refunds drop constraint if exists booking_refunds_status_check;
alter table market.booking_refunds add constraint booking_refunds_status_check
...
alter table market.seller_settlements add constraint seller_settlements_status_check
  check (status in ('scheduled','pending','processing','settled','failed','refund_pending','cancelled'));
...
begin
  ...
  if r.booking_id is not null and r.status='refunded' then
    select * into o from market.orders where id=r.order_id;
    return query select v.id,r.order_id,o.buyer_id,r.payment_provider,o.stripe_session_id,r.amount_gross,coalesce(v.deposit_gross,0),true;
    return;
  end if;

  if v.status not in ('pending_payment','confirmed') then raise exception 'Tej rezerwacji nie można zwrócić'; end if;
  ...
  if exists(select 1 from market.seller_settlements s where s.order_id=o.id and s.status='processing') then raise exception 'Wypłata sprzedawcy jest właśnie przetwarzana — spróbuj ponownie po zakończeniu rozliczenia'; end if;
  ...
  update market.seller_settlements set status='refund_pending',last_error=null,updated_at=now()
  ...
  on conflict(booking_id) do update set order_id=excluded.order_id,status='preparing',amount_gross=excluded.amount_gross,payment_provider=excluded.payment_provider,last_error=null,updated_at=now();
...
begin
  ...
  update market.seller_settlements set status=case when available_at is not null then 'scheduled' else 'pending' end,last_error=null,updated_at=now()
...
returns jsonb language plpgsql security definer set search_path=''
as $$
declare r market.booking_refunds%rowtype; v market.bookings%rowtype;
begin
  perform pg_advisory_xact_lock(hashtextextended('booking-refund:'||p_booking::text,42));
  select * into r from market.booking_refunds where booking_id=p_booking for update;
  ...
  if r.status<>'bonuses_reversed' then raise exception 'Bonusy nie zostały cofnięte przed finalizacją zwrotu'; end if;
  select * into v from market.bookings where id=p_booking for update;
  if v.id is null then raise exception 'Nie znaleziono rezerwacji'; end if;
  update market.bookings set status='cancelled',cancelled_at=coalesce(cancelled_at,now()),hold_expires_at=null,
...
revoke all on function market.booking_refund_finalize(uuid,text) from public,anon,authenticated;
