...
-- DROP is intentional: an earlier live hotfix used a different OUT row type.
...
begin
  ...
  if r.booking_id is not null and r.status='refunded' then
    ...
    return query select v.id,o.id,o.buyer_id,o.payment_provider,o.stripe_session_id,r.amount_gross,coalesce(v.deposit_gross,0),true;
    return;
  end if;
...
begin
  ...
  update market.ambassador_commission_outbox set status='reversed',last_error=null,updated_at=now() where order_id=r.order_id and status in ('ready','sent','failed','pending_vat','pending_identity');
