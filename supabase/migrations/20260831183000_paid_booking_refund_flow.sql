-- Paid booking cancellation/refund safety and audit state.
...
create table if not exists market.booking_refunds (
  ...
  status text not null check (status in ('preparing','blocked_bonus','bonuses_reversed','payment_failed','refunded','finalize_failed')),
...
begin
  ...
  if v.starts_at <= now() then raise exception 'Nie można automatycznie anulować opłaconej rezerwacji po rozpoczęciu terminu'; end if;
  ...
  if o.status <> 'paid' then raise exception 'Zamówienie nie jest w stanie opłaconym'; end if;
