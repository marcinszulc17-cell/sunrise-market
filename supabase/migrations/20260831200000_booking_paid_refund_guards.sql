...
alter table market.ambassador_commission_outbox add constraint ambassador_commission_outbox_status_check check (status = any(array['ready'::text,'pending_vat'::text,'pending_identity'::text,'sent'::text,'failed'::text,'reversed'::text]));
alter table market.seller_settlements drop constraint if exists seller_settlements_status_check;
alter table market.seller_settlements add constraint seller_settlements_status_check check (status = any(array['scheduled'::text,'pending'::text,'settled'::text,'failed'::text,'cancelled'::text]));
...
begin
  ...
  insert into market.booking_refunds(booking_id,order_id,status,amount_gross,payment_provider,last_error,updated_at) values(v.id,o.id,'preparing',o.total_gross,o.payment_provider,null,now())
...
begin
  ...
  update market.bookings set status='cancelled',cancelled_at=coalesce(cancelled_at,now()),hold_expires_at=null,
    ...
    deposit_resolution_note=case when coalesce(deposit_gross,0)>0 then 'Zwrot pełnej płatności przy anulowaniu rezerwacji' else deposit_resolution_note end, updated_at=now() where id=p_booking;
