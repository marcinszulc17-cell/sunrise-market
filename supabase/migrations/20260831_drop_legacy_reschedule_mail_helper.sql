-- Repair a late live-database drift that recreated a second reschedule-mail
-- helper/trigger after the canonical booking mail pipeline had been merged.
-- Keep one enqueue helper, one trigger and one idempotency model.

update market.booking_mail_outbox
set event_key = event_type
where event_type <> 'rescheduled'
  and (event_key is null or event_key='default');

update market.booking_mail_outbox
set event_key = 'rescheduled:' || id::text
where event_type='rescheduled'
  and (event_key is null or event_key='default');
...
begin
  ...
  select bo.timezone into v_timezone
  ...
  if coalesce(v_buyer_email,'')<>'' then
    ...
    ) values(b.id,p_event,v_event_key,'buyer',v_buyer_email,v_payload)
  ...
  if coalesce(v_seller_email,'')<>'' then
    insert into market.booking_mail_outbox(
      booking_id,event_type,event_key,recipient_type,recipient_email,payload
    ) values(b.id,p_event,v_event_key,'seller',v_seller_email,v_payload)
    on conflict do nothing;
  end if;
end;
$$;
...
-- Remove the divergent dedicated reschedule-mail path.
