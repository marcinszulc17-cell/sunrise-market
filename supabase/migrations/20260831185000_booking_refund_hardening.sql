-- Hardening for paid booking refunds after the initial full-refund rollout.
-- Automatic refunds are only allowed before the booking starts, for a fully paid order,
-- before seller settlement, and while any refundable deposit is still held.

-- Earlier rollout versions used a different TABLE return contract. PostgreSQL cannot
-- CREATE OR REPLACE a function when OUT columns change, so intentionally drop first.
