...
- **Parytet z Bookingiem — stan 2026-09-11/12** (migracje `20260911150000`–`20260911170000`):
  - **Synchronizacja iCal w obie strony.** Eksport: `/api/ical?offer=&token=` na sunrisemarket.pl
    (publiczny, chroniony `booking_offers.ical_token`, cache 60 s) — oddaje TYLKO rezerwacje i blokady
    ręczne, nigdy blokad zaciągniętych z cudzych kalendarzy (inaczej portale odbijałyby sobie terminy
    w nieskończoność). Import: edge fn `ical` + `booking_ical_feeds`, cron `market-ical-sync` co 30 min;
    blokada, której nie ma już u źródła, jest kasowana. Panel: `SellerIcalPanel` w `/sprzedawca/rezerwacje`.
    **Eksport NIE jest edge function** — portale pobierają anonimowo, a `verify_jwt` zostaje włączone (§reguły).
  - **Cena za osobę**: `booking_offers.price_mode` (`per_night` | `per_person`), `bookings.guests`.
    Wycena `booking_stay_quote`, blokada `create_booking_hold_v4` (wizyty przechodzą przez v2).
  - **Opłaty**: `city_tax_per_person_night` (miejscowa), `pet_fee_per_night`, `base_guests` +
    `extra_person_fee_per_night` (dopłata za osobę ponad limit w cenie). Wszystkie wchodzą do `fees_gross`
    i są pokazywane gościowi osobno przed płatnością. **Ani prowizja platformy, ani cashback NIE obejmują
    opłaty miejscowej** (decyzja właściciela 2026-09-12): `bookings.city_tax_gross` jest odejmowane od
    podstawy w `checkout_booking` i w edge fn `checkout` (wersja 64). To danina dla gminy, którą właściciel
    tylko pobiera i odprowadza — nie jego przychód, więc cała trafia do niego. **Pozostałe opłaty
    (sprzątanie, zwierzę, dopłata za osobę) liczą się normalnie do jednego i do drugiego** — to realny
    przychód obiektu. Kaucja poza podstawą jak dotąd (zwrotna). Pole opłaty miejscowej jest opcjonalne:
    właściciel, który wliczy daninę w cenę doby, ma `city_tax_gross = 0` i wszystko liczy się od pełnej kwoty.
    **Kupującemu nie tłumaczymy podatków** — widzi rozpisane pozycje i jedną linijkę „Cashback 3%: +X zł
    na portfel · bez opłaty miejscowej i kaucji". Uzasadnienie zostaje w Regulaminie §7.3.
  - **Polityka anulowania**: `cancellation_policy` (`flexible` | `moderate` | `strict` | `non_refundable`),
    `booking_cancellation_quote` liczy zwrot, `buyer_booking_cancel_prepare` + edge fn `booking-buyer-cancel`
    wykonują go. Kaucja i sprzątanie wracają zawsze; zatrzymany czynsz zostaje u właściciela, a jego wypłata
    i nasza prowizja maleją proporcjonalnie (`booking_refund_finalize_partial`). Cashback i prowizja
    ambasadorska cofane w całości — pobytu nie było.
  - **Struktura i zasady pobytu**: sypialnie, łazienki, łóżka (jsonb), metraż, cisza nocna, palenie, imprezy,
    dzieci, zwierzęta, odbiór kluczy, dodatkowe zasady (`seller_booking_save_stay_v2`, `offer_stay_details`).
  - **Gotowość przed publikacją**: `stay_readiness` + twarda blokada w `configure_booking_offer` —
    nocleg nie zostanie opublikowany poniżej **5 zdjęć** i **200 znaków opisu**.
  - **Mapa i dojazd**: `latitude`/`longitude`/`directions` podaje właściciel (nie geokodujemy),
    pinezka pokazuje OKOLICĘ; dokładny adres gość dostaje w potwierdzeniu rezerwacji.
- Do zbudowania po stronie UI: pasek „dokąd / termin / liczba osób" na `/rezerwacje`.
