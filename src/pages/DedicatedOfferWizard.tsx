...
export default function DedicatedOfferWizard() {
  ...
  useEffect(() => {
    setPurchaseMode(requestedMode && modes.includes(requestedMode) ? requestedMode : (type === "usluga" ? "appointment" : "purchase"));
  }, [type, requestedMode]);
    ...
    try {
      ...
      if (purchaseMode !== "purchase") {
        await configureBookingOffer({
          ...
          minNoticeHours: 2,
          maxAdvanceDays: 365,
          maxUnits: purchaseMode === "daily" ? 60 : 1,
          pricePerUnit: price,
          active: true,
        });
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_300px]">
      <Card>
        ...
        {step === 2 && <div className="space-y-4">
          ...
          <div className="flex items-center justify-between"><span className="font-semibold">Opis</span><button type="button" onClick={aiDescription} disabled={aiBusy} className="rounded-lg px-3 py-1.5 text-xs font-semibold" style={{ border: "1px solid var(--line)" }}>{aiBusy ? "Tworzę…" : "✨ Napisz opis AI"}</button></div>
        ...
        {step === 3 && <div className="space-y-5">
          <div className="rounded-xl p-4" style={{ background: "rgba(122,184,154,.08)", border: "1px solid rgba(122,184,154,.22)" }}><div className="font-semibold">{modeName(purchaseMode)}</div><div className="mt-1 text-xs" style={{ color: "var(--mut)" }}>{modeDescription(purchaseMode)}</div></div>
          <label className="block text-sm"><span className="mb-1 block" style={{ color: "var(--mut)" }}>{purchaseMode === "daily" ? "Cena za dzień" : purchaseMode === "appointment" ? "Cena terminu / usługi" : "Cena brutto"} *</span><input type="number" min="0" step="0.01" className={inputClass} style={inputStyle} value={price || ""} onChange={e => setPrice(Number(e.target.value))}/></label>
          ...
          <button onClick={publish} disabled={busy} className="w-full rounded-xl py-3 font-bold text-black disabled:opacity-50" style={{ background: "linear-gradient(135deg,#C8965A,#E8C896)" }}>{busy ? "Publikuję…" : purchaseMode === "purchase" ? "Opublikuj ofertę" : "Opublikuj i ustaw kalendarz →"}</button>
        </div>}
