...
export default function DedicatedOfferWizard() {
  ...
  async function publish() {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc("create_offer_v2", {
        ...
        p_attributes: { ...attrs, ...(location.trim() ? { location: location.trim() } : {}), ...(await serviceAreaAttrs(location, radiusKm)), offer_type: type, purchase_mode: purchaseMode, full_vat_invoice: fullVatInvoice, vat_rate: Number(vatRate) },
      });
  ...
  return <Shell>
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_300px]">
      <Card>
        <div className="mb-6 grid grid-cols-3 gap-2 text-center text-xs">{["Dane", "Zdjęcia i opis", "Cena i publikacja"].map((x, i) => <button key={x} onClick={() => setStep(i + 1)} className="rounded-lg px-2 py-2 font-semibold" style={{ background: step === i + 1 ? "rgba(232,137,26,.18)" : "var(--glass)", color: step === i + 1 ? "var(--gold)" : "var(--mut)" }}>{i + 1}. {x}</button>)}</div>
