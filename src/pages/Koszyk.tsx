...
export default function Koszyk() {
  ...
  const [topupMax, setTopupMax] = useState(25000);
  ...
  useEffect(() => {
    ...
    getMarketConfig().then((c) => { setCashbackRate(c.cashbackRate); setTopupMax(c.topupMaxPln); });
  }, []);
  useEffect(() => {
  ...
  // Brak większy niż limit jednorazowego doładowania — portfel nie ma sensu, od razu karta (Stripe) z pełnym cashbackiem.
  const topupTooBig = shortfall > topupMax;
  ...
  async function topupAndPay(amt: number) {
    ...
    if (amount > topupMax) { setMsg(`Jednorazowe doładowanie portfela to maksymalnie ${zl(topupMax)}. Przy tej kwocie zapłać kartą — cashback ${pkt(cashback)} pkt dostaniesz tak samo.`); return; }
      ...
      <main className="mx-auto max-w-4xl px-4 py-8">
        ...
        {done ? (
          <div className="rounded-2xl p-6" style={{ background: "var(--glass)", border: "1px solid rgba(122,184,154,.4)" }}>
            ...
            <p className="text-sm" style={{ color: "var(--ink)" }}>{done.method === "card" ? <>Zapłacono {done.paid > 0 ? <b>{zl(done.paid)}</b> : null} kartą (Stripe). Cashback 3% trafi na Twój portfel Sunrise Pay zaraz po potwierdzeniu płatności przez Stripe (zwykle w ciągu minuty).</> : <>Zapłacono <b>{zl(done.paid)}</b> z portfela Sunrise Pay. Cashback <b style={{ color: "var(--green)" }}>+{pkt(done.cashback)} pkt</b> trafił na portfel{done.balance != null && <> — nowe saldo: <b>{zl(done.balance)}</b></>}.</>}</p>
            <p className="text-xs mt-2" style={{ color: "var(--mut)" }}>Nr zamówienia: {done.order}</p>
            <div className="flex gap-3 mt-4"><a href="/" className="rounded-xl px-5 py-2 font-semibold text-black" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)" }}>Kupuj dalej</a><a href="/zamowienia" className="rounded-xl px-5 py-2 text-sm" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>Moje zamówienia</a><a href="/portfel" className="rounded-xl px-5 py-2 text-sm" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>Portfel</a></div>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <div className="md:col-span-2 flex flex-col gap-5">
              {presentLanes.map((lane) => {
                ...
                return <div key={lane} className="rounded-2xl p-4" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
                  <div className="flex items-center justify-between mb-1"><div className="font-semibold flex items-center gap-2">{meta.icon} {meta.title}</div>{etaSample && <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "rgba(232,137,26,.12)", color: "var(--gold)" }}>⏱ {etaSample}</span>}</div>
                  <div className="text-xs mb-3" style={{ color: "var(--mut)" }}>{meta.note}</div>
                  <div className="flex flex-col gap-2">{(groups[lane] ?? []).map((i) => <div key={i.offer_id} className="flex items-center gap-3"><div className="flex-1"><a href={`/produkt/${i.offer_id}`} className="font-medium hover:text-amber-300">{cleanTitle(i.title)}</a>{i.variant && <div className="text-xs" style={{ color: "var(--gold)" }}>{i.variant}</div>}{(() => { const sub = subscriptionInfo(i.billing ? { subscription: { interval: i.billing } } : null, i.title); return sub ? <div className="text-xs" style={{ color: "#7FE7F0" }}>{zl(i.price)} {sub.priceSuffix} · 🔁 {sub.badge}, płatna z góry, odnawiana automatycznie</div> : <div className="text-xs" style={{ color: "var(--mut)" }}>{zl(i.price)} / szt.</div>; })()}</div><div className="flex items-center gap-2"><button onClick={() => setQty(i.offer_id, i.qty - 1)} className="w-8 h-8 rounded-lg" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>−</button><span className="w-8 text-center">{i.qty}</span><button onClick={() => setQty(i.offer_id, i.qty + 1)} className="w-8 h-8 rounded-lg" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>+</button></div><div className="w-24 text-right font-display text-lg font-semibold">{zl(i.price * i.qty)}</div><button onClick={() => removeItem(i.offer_id)} className="text-zinc-500 hover:text-rose-400">✕</button></div>)}</div>
                  <div className="mt-3 pt-3" style={{ borderTop: "1px solid var(--line)" }}><div className="text-xs mb-2" style={{ color: "var(--mut)" }}>Dostawa dla tej części</div><div className="flex flex-wrap gap-2">{opt.map((m) => <label key={m.code} className="flex items-center gap-2 text-sm cursor-pointer rounded-lg px-3 py-1.5" style={{ border: selected[lane] === m.code ? "1px solid rgba(232,137,26,.6)" : "1px solid var(--line)" }}><input type="radio" name={`ship-${lane}`} checked={selected[lane] === m.code} onChange={() => setSelected({ ...selected, [lane]: m.code })} />{m.name}<span style={{ color: freeShip && Number(m.price_gross) > 0 ? "var(--green)" : "var(--mut)" }}>{freeShip ? (Number(m.price_gross) === 0 ? "0 zł" : <><s style={{ opacity: .5 }}>{zl(m.price_gross)}</s> 0 zł</>) : (Number(m.price_gross) === 0 ? "0 zł" : zl(m.price_gross))}</span></label>)}</div></div>
                </div>;
              })}
            </div>
            ...
            <div className="rounded-2xl p-5 h-fit" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
              ...
              {balance != null && !enoughFunds && topupTooBig ? <div className="flex flex-col gap-2"><div className="rounded-lg px-3 py-2 text-sm" style={{ background: "rgba(232,137,26,.12)", color: "var(--gold)" }}>W portfelu brakuje <b>{zl(shortfall)}</b> — to więcej niż jednorazowe doładowanie ({zl(topupMax)}). Przy takiej kwocie najprościej zapłacić kartą, BLIK-iem lub P24 — cashback {pkt(cashback)} pkt i Ochrona Kupujących działają tak samo.</div><button onClick={payByCard} disabled={busy || resuming || !deliveryReady || !invoiceReady} className="w-full rounded-xl py-3 font-semibold text-black disabled:opacity-50" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)" }}>{busy ? "Przekierowuję do płatności…" : `💳 Zapłać kartą / BLIK / P24 · ${zl(grand)} →`}</button>{!deliveryReady && <div className="text-xs" style={{ color: "var(--gold)" }}>Najpierw uzupełnij adres dostawy powyżej.</div>}{!invoiceReady && <div className="text-xs" style={{ color: "var(--gold)" }}>Uzupełnij dane do faktury powyżej.</div>}<a href="/portfel" className="text-center text-xs underline" style={{ color: "var(--mut)" }}>albo doładuj portfel w kilku transzach w aplikacji MySunrise</a></div> : balance != null && !enoughFunds ? <div className="flex flex-col gap-2"><div className="rounded-lg px-3 py-2 text-sm" style={{ background: "rgba(232,137,26,.12)", color: "var(--gold)" }}>Brakuje <b>{zl(shortfall)}</b> w portfelu. Zapłać punktami albo doładuj — bez wychodzenia z koszyka:</div>{points >= Math.ceil(shortfall) && <><button onClick={redeemAndPay} disabled={busy || resuming || !deliveryReady || !invoiceReady} className="w-full rounded-xl py-3 font-semibold text-black disabled:opacity-50" style={{ background: "linear-gradient(135deg,#7AB89A,#12b981)" }}>Wymień {pkt(Math.ceil(shortfall))} pkt i zapłać →</button><div className="text-center text-xs" style={{ color: "var(--mut)" }}>masz {pkt(points)} pkt · albo doładuj kartą:</div></>}
                <div className="flex flex-wrap items-center gap-2">{topupSuggestions.map((v) => <button key={v} onClick={() => setTopupAmount(String(v))} className="rounded-lg px-3 py-1.5 text-sm" style={{ border: topupDisplay === String(v) ? "1px solid rgba(232,137,26,.6)" : "1px solid var(--line)", background: "var(--glass)" }}>{zl(v)}</button>)}<input type="number" min={ceilShort} step={1} inputMode="numeric" value={topupDisplay} onChange={(e) => setTopupAmount(e.target.value)} className="w-24 rounded-lg px-3 py-2 text-sm outline-none" style={{ background: "var(--glass)", border: "1px solid var(--line)" }} /><span className="text-xs" style={{ color: "var(--mut)" }}>min {zl(shortfall)}</span></div>
                <div className="text-xs" style={{ color: "var(--mut)" }}>Większe doładowanie = mniej opłat i zapas w portfelu na kolejne zakupy.</div><button onClick={() => topupAndPay(effectiveTopup)} disabled={busy || resuming || !deliveryReady || !invoiceReady} className="w-full rounded-xl py-3 font-semibold text-black disabled:opacity-50" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)" }}>{busy ? "Przekierowuję do płatności…" : `Doładuj ${zl(effectiveTopup)} i zapłać →`}</button>{!deliveryReady && <div className="text-xs" style={{ color: "var(--gold)" }}>Najpierw uzupełnij adres dostawy powyżej.</div>}{!invoiceReady && <div className="text-xs" style={{ color: "var(--gold)" }}>Uzupełnij dane do faktury powyżej.</div>}<a href="/portfel" className="text-center text-xs underline" style={{ color: "var(--mut)" }}>albo doładuj w aplikacji MySunrise</a><div className="text-center text-xs pt-1" style={{ color: "var(--mut)" }}>— albo bez portfela —</div><button onClick={payByCard} disabled={busy || resuming || !deliveryReady || !invoiceReady} className="w-full rounded-xl py-3 font-semibold disabled:opacity-50" style={{ background: "var(--glass)", border: "1px solid var(--line)", color: "var(--ink)" }}>💳 Zapłać kartą (Stripe) · cashback {pkt(cashback)} pkt</button></div> : <div className="flex flex-col gap-2"><button onClick={pay} disabled={busy || resuming || !deliveryReady || !invoiceReady || (balance != null && !enoughFunds)} className="w-full rounded-xl py-3 font-semibold text-black disabled:opacity-50" style={{ background: "linear-gradient(135deg,#E8891A,#A97B42)" }}>{resuming ? "Dokańczam zamówienie…" : busy ? "Płacę…" : !invoiceReady ? "Uzupełnij dane do faktury" : `Zapłać saldem · wróci ${pkt(cashback)} pkt`}</button><button onClick={payByCard} disabled={busy || resuming || !deliveryReady || !invoiceReady} className="w-full rounded-xl py-2.5 text-sm font-semibold disabled:opacity-50" style={{ background: "var(--glass)", border: "1px solid var(--line)", color: "var(--ink)" }}>💳 Zapłać kartą (Stripe) · cashback {pkt(cashback)} pkt</button></div>}
              {cart.some((i) => Boolean(subscriptionInfo(i.billing ? { subscription: { interval: i.billing } } : null, i.title))) && <div className="mt-3 rounded-xl px-3 py-2 text-xs" style={{ background: "rgba(56,224,240,.08)", border: "1px solid rgba(56,224,240,.2)", color: "var(--ink)" }}><b>🔁 W koszyku masz subskrypcję.</b> Subskrypcja jest opłacana z góry za każdy okres (miesiąc) i odnawia się automatycznie, żeby usługa działała bez przerw. Subskrypcje rozliczane są kartą przez Stripe.</div>}
              <p className="text-xs mt-3" style={{ color: "var(--mut)" }}>Cashback {cashbackPct}% wraca na portfel Sunrise Pay przy każdej metodzie płatności — saldem, kartą, BLIK-iem czy P24. Subskrypcje rozliczane są przez Stripe (karta).</p>
            </div>
          </div>
          {recs.length > 0 && <section className="mt-8"><h2 className="font-display text-xl font-semibold mb-4">Może Cię zainteresować</h2><div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))" }}>{recs.map((r) => <div key={r.offer_id} className="rounded-2xl p-3 flex flex-col" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}><a href={`/produkt/${r.offer_id}`} className="font-medium text-sm hover:text-amber-300 flex-1">{r.title}</a><div className="flex items-center justify-between mt-2"><span className="font-display text-lg font-semibold">{zl(r.price_gross)}</span><button onClick={() => addToCart({ offer_id: r.offer_id, title: r.title, price: Number(r.price_gross) })} className="text-xs font-semibold px-3 py-1.5 rounded-lg text-black" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)" }}>+ Dodaj</button></div></div>)}</div></section>}
        </>}
      </main>
    </div>
  );
