...
import { Ico, GOLD_GRAD, CARD, HomeFooter, timeAgo, ViewToggle, useViewMode } from "../components/home/HomeShared";
...
export default function Obserwowane() {
  ...
  // Siatka / lista — wg makiet właściciela 2026-09-10.
  const [view, setView] = useViewMode("ulubione");
      ...
      <div className="mt-5 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <aside className="hidden h-fit rounded-2xl p-4 lg:block" style={CARD}>
          ...
          {authed && <div className="mt-5 border-t pt-4 text-sm" style={{ borderColor: "var(--line)" }}>
            ...
            {/* „Ostatnio oglądane" to WŁASNA historia kupującego trzymana w jego przeglądarce (lib/recent),
                a nie statystyka oferty — tych sprzedawcowych liczb kupującemu nie pokazujemy (§10). */}
            <Link to="/sklep" className="flex items-center justify-between py-1"><span className="flex items-center gap-2" style={{ color: "var(--mut)" }}><Ico name="search" size={16} stroke="var(--gold)" />Ostatnio oglądane</span><b>{getRecent().length}</b></Link>
            <Link to="/zamowienia" className="flex items-center justify-between py-1"><span className="flex items-center gap-2" style={{ color: "var(--mut)" }}><Ico name="bag" size={16} stroke="var(--gold)" />Moje zakupy</span><b style={{ color: "var(--mut)" }}>›</b></Link>
            <Link to="/rezerwacje" className="flex items-center justify-between py-1"><span className="flex items-center gap-2" style={{ color: "var(--mut)" }}><Ico name="calendar" size={16} stroke="var(--gold)" />Moje rezerwacje</span><b style={{ color: "var(--mut)" }}>›</b></Link>
        ...
        <section>
          ...
          : <div className={view === "list" ? "mt-5 grid gap-3" : "mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"}>{sorted.map((x) => <article key={x.offer_id} className={`group relative overflow-hidden rounded-2xl transition hover:-translate-y-0.5 ${view === "list" ? "flex sm:items-stretch" : "flex flex-col"}`} style={CARD}>
              <Link to={`/produkt/${x.offer_id}`} className={view === "list" ? "block aspect-[4/3] w-32 shrink-0 overflow-hidden sm:w-56" : "block aspect-[4/3] w-full overflow-hidden"} style={{ background: "var(--header)" }} tabIndex={-1} aria-hidden="true">{x.image_url ? <img src={x.image_url} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" /> : <div className="grid h-full place-items-center text-3xl">🛍️</div>}</Link>
