  ...
  return <main className="min-h-screen" style={{ background: "var(--bg)", color: "var(--ink)" }}>
    ...
    <div className="mx-auto max-w-4xl px-4 py-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><h1 className="text-2xl font-semibold">Ulubione</h1><p className="mt-1 text-sm" style={{ color: "var(--mut)" }}>Oferty, które oznaczyłeś ♡. Powiadomimy Cię, gdy cena spadnie.</p></div>
        {compare.length > 0 && <Link to="/porownaj" className="rounded-xl px-4 py-2 text-sm font-semibold" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)", color: "#101012" }}>Porównaj ({compare.length}) →</Link>}
      </div>
      ...
      {authed === false ? <div className="mt-5 rounded-2xl p-6" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}><div className="font-semibold">Zaloguj się, aby zobaczyć ulubione</div><p className="mt-1 text-sm" style={{ color: "var(--mut)" }}>Obserwowane oferty zapisujemy na Twoim koncie, więc są dostępne na każdym urządzeniu.</p><a href={`/login?next=${encodeURIComponent("/obserwowane")}`} className="mt-4 inline-block rounded-xl px-4 py-2 text-sm font-semibold" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)", color: "#101012" }}>Zaloguj się</a></div>
