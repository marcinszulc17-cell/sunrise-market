  ...
  return <div className="min-h-screen">
    <header className="sticky top-0 z-20 backdrop-blur" style={{ background: "var(--header)", borderBottom: "1px solid var(--line)" }}>
      <div className="mx-auto flex max-w-5xl items-center gap-3 px-4 py-3">
        <a href="/"><img src="/logo-sunrise-market.png" alt="Sunrise Market" className="h-8 w-auto rounded-lg bg-white p-1" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} /></a>
      ...
      {o && <>
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-4">
            ...
            {isBooking && <div className="rounded-xl px-3 py-2 text-sm" style={{ background: "rgba(56,224,240,.08)", border: "1px solid rgba(56,224,240,.2)" }}><b>{purchaseMode === "daily" ? "🗓️ Wynajem na dni" : "📅 Rezerwacja terminu"}</b><div className="mt-1 text-xs" style={{ color: "var(--mut)" }}>{purchaseMode === "daily" ? "Wybierz datę rozpoczęcia i zakończenia. Cena zostanie policzona dla wybranego okresu." : "Wybierz dostępny dzień i godzinę, a następnie opłać rezerwację."}</div></div>}
            ...
            <div className="flex flex-wrap items-center gap-3"><div className="font-display text-4xl font-bold">{zl(o.price_gross)}{purchaseMode === "daily" ? <span className="text-base font-medium"> / dzień</span> : purchaseMode === "appointment" ? <span className="text-base font-medium"> / termin</span> : null}</div><span className="rounded-full px-3 py-1 text-sm font-semibold" style={{ background: "rgba(122,184,154,.12)", color: "var(--green)" }}>+{Math.round(o.price_gross * 0.03).toLocaleString("pl-PL")} pkt cashback</span></div>
            <div className="text-sm" style={{ color: o.stock > 0 ? "var(--green)" : "#F25CB0" }}>{o.stock > 0 ? (isBooking ? `Dostępne zasoby: ${o.stock}` : `Dostępne: ${o.stock} szt.`) : "Chwilowo niedostępne"}</div>
            ...
            <div className="flex flex-wrap gap-2 pt-1">{(isBooking ? ["Ochrona płatności", "Płatna rezerwacja", "Płatność Sunrise Pay", "Cashback na portfel"] : ["Ochrona kupującego", "Zwrot 14 dni", "Płatność Sunrise Pay", "Cashback na portfel"]).map(t => <span key={t} className="rounded-lg px-2.5 py-1 text-xs" style={{ background: "var(--glass)", border: "1px solid var(--line)", color: "var(--mut)" }}>✓ {t}</span>)}</div>
