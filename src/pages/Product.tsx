      ...
      {o && <>
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">{A.promo?.old_price && <div className="text-xl font-semibold line-through" style={{ color: "var(--mut)" }}>{zl(A.promo.old_price)}</div>}<div className="font-display text-4xl font-bold">{zl(o.price_gross)}{purchaseMode === "daily" ? <span className="text-base font-medium"> / dobę</span> : purchaseMode === "appointment" ? <span className="text-base font-medium"> / termin</span> : sub ? <span className="text-base font-medium"> {sub.priceSuffix}</span> : null}</div>{!sub && <span className="rounded-full px-3 py-1 text-sm font-semibold" style={{ background: "rgba(122,184,154,.12)", color: "var(--green)" }}>+{Math.round(o.price_gross * 0.03).toLocaleString("pl-PL")} pkt cashback</span>}{A.promo?.percent && <span className="rounded-full px-3 py-1 text-sm font-bold" style={{ background: "rgba(242,92,176,.16)", color: "#F8A8D2", border: "1px solid rgba(242,92,176,.35)" }}>PROMOCJA −{A.promo.percent}%{A.promo.until ? ` do ${new Date(A.promo.until).toLocaleDateString("pl-PL")}` : ""}</span>}</div>
            ...
            <div className="flex flex-wrap gap-2 pt-1">{(isBooking ? ["Ochrona płatności", "Płatna rezerwacja", "Płatność Sunrise Pay", "Cashback na portfel"] : sub ? ["Płatność kartą", "Odnawiana co miesiąc", "Rezygnacja w każdej chwili"] : ["Ochrona kupującego", "Zwrot 14 dni", "Płatność Sunrise Pay", "Cashback na portfel"]).map(t => <span key={t} className="rounded-lg px-2.5 py-1 text-xs" style={{ background: "var(--glass)", border: "1px solid var(--line)", color: "var(--mut)" }}>✓ {t}</span>)}</div>
