      ...
      {o && <>
        ...
        <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">{A.promo?.old_price && <div className="text-xl font-semibold line-through" style={{ color: "var(--mut)" }}>{zl(A.promo.old_price)}</div>}<div className="text-4xl font-extrabold" style={{ color: "var(--gold)" }}>{zl(o.price_gross)}{purchaseMode === "daily" ? <span className="text-base font-medium" style={{ color: "var(--mut)" }}> {stay?.price_mode === "per_person" ? "/ osobę za dobę" : "/ dobę"}</span> : purchaseMode === "appointment" ? <span className="text-base font-medium" style={{ color: "var(--mut)" }}> / termin</span> : sub ? <span className="text-base font-medium" style={{ color: "var(--mut)" }}> {sub.priceSuffix}</span> : null}</div></div>
        ...
        <LocationMap location={typeof (o.attributes as any)?.location === "string" ? (o.attributes as any).location : null} radiusKm={Number((o.attributes as any)?.service_radius_km) || null} kind={locationKind(o.category_slug, purchaseMode)} className="mt-6 max-w-xl" />
