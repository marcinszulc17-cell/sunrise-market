...
export default function Rezerwacje() {
  ...
  return <main className="min-h-screen px-4 py-8" style={{ background: "var(--bg)", color: "var(--ink)" }}>
    <div className="mx-auto max-w-4xl">
      ...
      {notice && <p className="mb-5 rounded-2xl p-4 text-sm" style={{ background: "rgba(232,137,26,.10)", border: "1px solid rgba(232,137,26,.25)", color: "var(--gold)" }}>{notice}</p>}
      ...
      <div className="grid gap-3">{visibleRows.map((r) => {
        ...
        return <article key={r.id} className="rounded-2xl p-5" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
          ...
          {r.status === "pending_payment" && r.paid_at && <p className="mt-3 rounded-xl px-3 py-2 text-xs" style={{ background: "rgba(232,137,26,.08)", border: "1px solid rgba(232,137,26,.2)", color: "var(--mut)" }}>Płatność jest zaksięgowana, a termin nadal zarezerwowany dla Ciebie. Sprzedawca musi tylko zaakceptować rezerwację.</p>}
