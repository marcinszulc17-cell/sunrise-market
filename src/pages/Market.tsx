  ...
  return (
    <article className="card-glow rounded-2xl overflow-hidden flex flex-col" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="font-display text-2xl font-semibold">{cena(o.price_gross)}{sub && <span className="text-sm font-medium" style={{ color: "var(--mut)" }}> {sub.priceSuffix}</span>}</div>
            ...
            {recent.map((r) => (
              <a key={r.offer_id} href={`/produkt/${r.offer_id}`} className="shrink-0 w-40 rounded-2xl overflow-hidden card-glow"
                ...
                <div className="p-2">
                  ...
                  <div className="font-display font-semibold text-sm mt-1">{cena(r.price_gross)}</div>
                </div>
              </a>
            ))}
          </div>
