...
export default function SpecializedProduct() {
  ...
  return <div className="min-h-screen" style={{ background: "var(--bg)", color: "var(--ink)" }}>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_390px]">
        <section>
          <div
            ...
            style={{ border: "1px solid var(--line)", background: "rgba(0,0,0,.16)" }}
            ...
            onTouchEnd={(e)=>{
              ...
              const dx=end-start;
              if(Math.abs(dx)>45) dx<0?nextPhoto():prevPhoto();
            }}
          >
            ...
            {imgs.length>1&&<>
              <button type="button" aria-label="Poprzednie zdjęcie" onClick={prevPhoto} className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-black/65 px-4 py-3 text-2xl text-white opacity-90 backdrop-blur transition hover:bg-black/80 sm:opacity-0 sm:group-hover:opacity-100">‹</button>
              <button type="button" aria-label="Następne zdjęcie" onClick={nextPhoto} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-black/65 px-4 py-3 text-2xl text-white opacity-90 backdrop-blur transition hover:bg-black/80 sm:opacity-0 sm:group-hover:opacity-100">›</button>
              <div className="absolute bottom-3 right-3 rounded-full bg-black/65 px-3 py-1 text-xs text-white">{active+1}/{imgs.length}</div>
            </>}
    ...
    {lightboxOpen && mainImage && <div className="fixed inset-0 z-[70] bg-black/95" onMouseDown={(e)=>{if(e.target===e.currentTarget){setLightboxOpen(false);resetZoom();}}}>
      <div className="absolute left-1/2 top-4 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full bg-black/70 p-2 text-white backdrop-blur">
        <button type="button" onClick={()=>changeZoom(zoom-0.5)} className="rounded-full px-4 py-2 text-xl" aria-label="Pomniejsz">−</button>
        <button type="button" onClick={resetZoom} className="min-w-16 rounded-full px-3 py-2 text-sm">{Math.round(zoom*100)}%</button>
        <button type="button" onClick={()=>changeZoom(zoom+0.5)} className="rounded-full px-4 py-2 text-xl" aria-label="Powiększ">+</button>
      </div>
      ...
      {imgs.length>1&&<>
        <button type="button" onClick={prevPhoto} className="absolute left-3 top-1/2 z-20 -translate-y-1/2 rounded-full bg-black/70 px-5 py-4 text-3xl text-white">‹</button>
        <button type="button" onClick={nextPhoto} className="absolute right-3 top-1/2 z-20 -translate-y-1/2 rounded-full bg-black/70 px-5 py-4 text-3xl text-white">›</button>
        <div className="absolute bottom-4 left-1/2 z-20 -translate-x-1/2 rounded-full bg-black/70 px-4 py-2 text-sm text-white">{active+1}/{imgs.length}</div>
      </>}
      <div
        className="flex h-full w-full touch-none items-center justify-center overflow-hidden"
        style={{cursor:zoom>1?(dragging?"grabbing":"grab"):"zoom-in"}}
        onWheel={(e)=>{e.preventDefault();changeZoom(zoom+(e.deltaY<0?0.25:-0.25));}}
      ...
      >
        <img src={displayImageUrl(mainImage,2500,2000)} alt={o.title} draggable={false} className="max-h-[92vh] max-w-[96vw] select-none object-contain transition-transform duration-100" style={{transform:`translate(${pan.x}px, ${pan.y}px) scale(${zoom})`}} />
      </div>
    </div>}
