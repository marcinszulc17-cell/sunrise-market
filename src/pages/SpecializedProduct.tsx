        ...
        <aside className="min-w-0 lg:sticky lg:top-24 lg:h-fit">
          <div className="rounded-3xl p-5 shadow-2xl sm:p-6" style={{ background: "var(--glass)", border: "1px solid rgba(232,137,26,.22)" }}>
            ...
            {canBuy && <div className="mt-5">
              <button type="button" onClick={buyViaSunrise} className="w-full rounded-2xl py-3 text-center font-semibold text-black" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)" }}>Kup teraz · 🛡 Ochrona Kupujących</button>
            ...
            <div className={`${canBuy || isProperty ? "mt-3" : "mt-5"} grid gap-2`}>{(isCar || isProperty) && <button type="button" onClick={() => window.dispatchEvent(new Event("sunrise-open-interaction"))} className="flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-bold" style={{ background: "linear-gradient(135deg,#E8891A,#F5A623)", color: "#101012", boxShadow: "0 8px 24px rgba(232,137,26,.3)" }}>📅 {isCar ? "Umów oględziny / jazdę próbną" : "Umów prezentację"}</button>}<ShowPhoneButton offerId={String(id)} className="flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-semibold" /><MessageSellerButton offerId={String(id)} title={o.title} variant={canBuy ? "secondary" : "primary"} className="flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-semibold" /><button onClick={() => { setLeadOpen(true); setLeadDone(false); setLeadError(null); }} className="rounded-2xl py-3 text-center font-semibold" style={{ border: "1px solid rgba(232,137,26,.45)", color: "var(--gold)" }}>Zapytaj o ofertę (telefon / e-mail)</button><ShareOfferButton offerId={String(id)} title={o.title} className="rounded-2xl py-3 text-sm font-semibold" style={{ border: "1px solid var(--line)" }} /></div>
