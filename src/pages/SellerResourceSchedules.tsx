...
export default function SellerResourceSchedules(){
 ...
 function chooseResource(id:string){setSelected(id);const next=new URLSearchParams(sp);next.set("resource",id);setSp(next,{replace:true})}
 ...
 async function saveResource(){
  ...
  setBusy(true);setMsg("");
  const{error}=await supabase.schema("market").rpc("seller_booking_resource_upsert",{p_offer:null,p_id:selected,p_name:edit.name.trim(),p_kind:edit.kind,p_description:edit.description||null,p_active:edit.active});
  setBusy(false);
  if(error){setMsg(error.message);return;}
