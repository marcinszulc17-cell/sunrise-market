...
export default function CategoryPortal({mode}:{mode:Mode}){
  ...
  const searchSlug = car ? "motoryzacja-samochody-osobowe" : "nieruchomosci";
  ...
  return <main className="min-h-screen" style={{background:"var(--bg)",color:"var(--ink)"}}>
    ...
    {cats.length>0&&<section className="mx-auto max-w-7xl px-4 pb-4"><div className="flex gap-2 overflow-x-auto">{cats.map(c=><a key={c.id} href={`/szukaj?cat=${encodeURIComponent(c.slug)}&mode=${car?"car":"property"}`} className="whitespace-nowrap rounded-full px-4 py-2 text-sm" style={glass}>{c.name}</a>)}</div></section>}
    {searched&&<OfferSection title={`Wyniki wyszukiwania (${results.length})`} rows={results} car={car}/>}<OfferSection title="Najnowsze" rows={latest} car={car}/><OfferSection title={car?"Najtańsze auta":"Najniższa cena"} rows={cheap} car={car}/>
