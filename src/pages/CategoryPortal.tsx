...
export default function CategoryPortal({mode}:{mode:Mode}){
  ...
  return <main className="min-h-screen" style={{background:"var(--bg)",color:"var(--ink)"}}>
    ...
    {searched&&<OfferSection title={`Wyniki wyszukiwania (${results.length})`} rows={results} car={car} rate={cashbackRate}/>}<OfferSection title="Najnowsze" rows={latest} car={car} rate={cashbackRate}/><OfferSection title={car?"Najtańsze auta":"Najniższa cena"} rows={cheap} car={car} rate={cashbackRate}/>
