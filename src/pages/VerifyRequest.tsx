  ...
  <div className="mt-5 overflow-hidden rounded-3xl" style={{background:"var(--glass)",border:"1px solid var(--line)"}}>
   ...
   {err?<div className="rounded-xl p-4 text-sm text-red-400" style={{border:"1px solid rgba(239,68,68,.3)"}}>{err}</div>:r&&<>
    ...
    {r.status==="ready"&&<>
      ...
      {warnings.length>0&&<section className="mt-6 rounded-2xl p-5" style={{background:"rgba(200,150,90,.08)",border:"1px solid rgba(200,150,90,.22)"}}><h2 className="font-semibold">Na co zwrócić uwagę</h2><ul className="mt-3 space-y-2 text-sm leading-6">{warnings.map((w:string,i:number)=><li key={i}>• {w}</li>)}</ul></section>}
...
function coverageLabel(k:string){return ({vin_identity:"VIN",vin_decode:"VIN",cepik_reference:"CEPiK",public_registry:"Rejestr publiczny",extended_history:"Historia rozszerzona",exact_polish_history:"Historia PL",geoportal:"Geoportal",land_register:"Księga wieczysta"} as Record<string,string>)[k]||k.replaceAll("_"," ")}
