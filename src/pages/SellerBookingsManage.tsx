...
import { refundPaidBooking } from "../lib/bookingRefund";
...
export default function SellerBookingsManage() {
  ...
  async function refundBooking(r: Booking) {
    const deposit = Number(r.deposit_gross || 0);
    const paymentTotal = Number(r.amount_gross || 0) + deposit;
    const depositInfo = deposit > 0 ? ` W tej kwocie jest również kaucja ${pln(deposit)}.` : "";
    if (!window.confirm(`Anulować opłaconą rezerwację i zwrócić klientowi pełną płatność ${pln(paymentTotal)}?${depositInfo}\n\nSystem cofnie także cashback i prowizje z tego zamówienia.`)) return;
    setRefundBusyId(r.id);
    setMsg("");
    try {
      const result = await refundPaidBooking(r.id);
      setMsg(result.already ? "Zwrot tej rezerwacji był już wykonany. Status został odświeżony." : `Rezerwacja anulowana, a ${pln(Number(result.amount || paymentTotal))} zwrócono klientowi. Cashback i prowizje zostały cofnięte. ✅`);
      await load();
    } catch (e) {
      setMsg("Nie udało się anulować i zwrócić płatności: " + (e as Error).message);
    } finally {
        ...
        <section>
          ...
          <div className="space-y-3">
            {visible.map((r) => {
              ...
              const paidAwaitingApproval = r.status === "pending_payment" && !!r.paid_at;
              const displayStatus = paidAwaitingApproval ? "Opłacona — do akceptacji" : (statusLabel[r.status] || r.status);
              return <article id={`booking-${r.id}`} key={r.id} className="scroll-mt-24 rounded-2xl p-5" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <span className="rounded-full px-3 py-1 text-xs font-semibold" style={{ background: r.status === "confirmed" || paidAwaitingApproval ? "rgba(34,197,94,.12)" : "var(--header)", border: "1px solid var(--line)" }}>{displayStatus}</span>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {["held", "pending_payment", "confirmed"].includes(r.status) && !!r.paid_at && <button disabled={busy || rescheduleBusy || depositBusy || refundBusy} onClick={() => refundBooking(r)} className="rounded-xl px-3 py-2 text-sm font-semibold disabled:opacity-50" style={{ border: "1px solid rgba(239,68,68,.5)", color: "#fca5a5" }}>{refundBusy ? "Zwracam…" : "↩ Anuluj i zwróć"}</button>}
