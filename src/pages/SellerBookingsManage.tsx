...
export default function SellerBookingsManage() {
  ...
  async function refundBooking(r: Booking) {
    if (!r.paid_at) return;
    if (!window.confirm(`Anulować opłaconą rezerwację i zwrócić klientowi pełną płatność?\n\nKwota rezerwacji: ${pln(r.amount_gross)}${Number(r.deposit_gross || 0) > 0 ? `\nW tym kaucja: ${pln(Number(r.deposit_gross || 0))}` : ""}\n\nCashback i prowizje z tego zamówienia zostaną cofnięte.`)) return;
    setRefundBusyId(r.id);
    setMsg("");
    try {
      const { data, error } = await supabase.functions.invoke("booking-refund-action", { body: { booking_id: r.id } });
      if (error) throw error;
      if (!data?.ok) throw new Error(data?.message || data?.error || "Nie udało się wykonać zwrotu");
      setMsg(`Rezerwacja anulowana, a ${pln(Number(data.amount ?? r.amount_gross))} zostało zwrócone klientowi. Cashback i prowizje zostały cofnięte. ✅`);
      await load();
    } catch (e) {
            ...
            {visible.map((r) => {
              ...
              return <article id={`booking-${r.id}`} key={r.id} className="scroll-mt-24 rounded-2xl p-5" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <span className="rounded-full px-3 py-1 text-xs font-semibold" style={{ background: r.status === "confirmed" ? "rgba(34,197,94,.12)" : r.status === "pending_payment" && r.paid_at ? "rgba(200,150,90,.16)" : "var(--header)", border: "1px solid var(--line)" }}>{displayStatus}</span>
                </div>
