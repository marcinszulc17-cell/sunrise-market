...
export default function SellerBookingsManage() {
  ...
  async function cancelAndRefund(r: Booking) {
    ...
    if (!window.confirm(`Anulować tę opłaconą rezerwację i zwrócić klientowi ${pln(fullAmount)}?\n\nCashback i prowizje powiązane z tym zamówieniem zostaną cofnięte, a planowana wypłata sprzedawcy anulowana.`)) return;
    setRefundBusyId(r.id); setMsg("");
    try {
      ...
      setMsg(`Rezerwacja została anulowana, a klient otrzymał zwrot ${pln(Number(data.refunded ?? fullAmount))}. Cashback i prowizje zostały cofnięte. ✅`);
      await load();
    } catch (e) {
            ...
            {visible.map((r) => {
              ...
              return <article id={`booking-${r.id}`} key={r.id} className="scroll-mt-24 rounded-2xl p-5" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {["pending_payment", "confirmed"].includes(r.status) && !!r.paid_at && <button disabled={busy || rescheduleBusy || depositBusy || refundBusy} onClick={() => cancelAndRefund(r)} className="rounded-xl px-3 py-2 text-sm font-semibold disabled:opacity-50" style={{ border: "1px solid rgba(239,68,68,.55)", color: "#fca5a5" }}>{refundBusy ? "Zwracam…" : "Anuluj i zwróć"}</button>}
