...
export async function checkout(items: { offer_id: string; qty: number }[], shippingCodes?: string[], shipping?: ShipAddress, paymentMethod?: "wallet" | "card") {
  const { data, error } = await supabase.functions.invoke("checkout", { body: { items, shipping_codes: shippingCodes ?? [], shipping: shipping ?? null, payment_method: paymentMethod ?? "wallet" } });
  if (error) throw error;
  return data;
}
