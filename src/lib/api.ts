...
export async function adminShipments() {
  const { data, error } = await supabase.rpc("admin_shipments"); if (error) throw error; return data ?? [];
}
...
export async function genDescription(title: string, category?: string, mode?: string, extra?: { image_urls?: string[]; attributes?: Record<string, unknown>; current_description?: string }): Promise<string> {
  const { data, error } = await supabase.functions.invoke("gen-description", { body: { title, category, mode, ...(extra ?? {}) } });
