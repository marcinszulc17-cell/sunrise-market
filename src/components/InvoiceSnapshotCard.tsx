...
export default function InvoiceSnapshotCard({ invoice, compact = false, showNoInvoice = false }: Props) {
  if (!invoice?.requested) {
    ...
    return <div className="rounded-xl px-3 py-2 text-xs" style={{ background: "var(--header)", border: "1px solid var(--line)", color: "var(--mut)" }}>🧾 Klient nie wybrał faktury VAT dla tego zamówienia.</div>;
  }
  ...
  return <div className={`rounded-2xl ${compact ? "p-3" : "p-4"}`} style={{ background: "rgba(200,150,90,.07)", border: "1px solid rgba(200,150,90,.24)" }}>
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="text-sm font-semibold" style={{ color: "var(--gold)" }}>🧾 Faktura VAT</div>
    ...
    <div className="mt-2 text-[10px]" style={{ color: "var(--mut)" }}>To historyczny snapshot zamówienia — nie jest edytowany po zakupie.</div>
  </div>;
}
