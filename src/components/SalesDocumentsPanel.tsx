...
export default function SalesDocumentsPanel({ orderId, mode, invoiceRequested = false }: Props) {
  ...
  if (mode === "buyer" && !loading && !invoiceRequested && documents.length === 0) return null;
  ...
  return <section className="mt-4 rounded-xl p-4" style={{ background: "var(--header)", border: "1px solid var(--line)" }}>
        ...
        <div className="mt-1 text-xs" style={{ color: "var(--mut)" }}>
          {mode === "seller"
            ? invoiceRequested
              ...
              : "Sunrise Market nie wystawia dokumentów za Ciebie. Jeśli chcesz przekazać klientowi dokument sprzedaży, dodaj gotowy PDF."
            : invoiceRequested
              ? "Jeśli sprzedawca wystawi dokument, pojawi się tutaj przy tym zamówieniu."
              : "Dokumenty przekazane przez sprzedawcę są dostępne tylko dla stron tego zamówienia."}
        </div>
      </div>
      {mode === "seller" && <button type="button" onClick={() => setAdding(v => !v)} className="rounded-xl px-3 py-2 text-xs font-semibold" style={{ border: "1px solid var(--line)", background: "var(--glass)" }}>
        {adding ? "Anuluj" : "+ Dodaj dokument PDF"}
      </button>}
    </div>

    {message && <div className="mt-3 text-xs" style={{ color: message.startsWith("Dokument został") ? "var(--green)" : "#fca5a5" }}>{message}</div>}

    {mode === "seller" && adding && <div className="mt-4 grid gap-3 rounded-xl p-4 sm:grid-cols-2" style={{ background: "var(--glass)", border: "1px solid var(--line)" }}>
      <label className="text-xs">Typ dokumentu
        <select value={documentType} onChange={e => setDocumentType(e.target.value)} className="mt-1 w-full rounded-lg px-3 py-2 text-sm" style={{ background: "var(--bg)", border: "1px solid var(--line)" }}>
          ...
          <option value="receipt">Rachunek / paragon</option>
          <option value="other">Inny dokument</option>
        </select>
      </label>
      <label className="text-xs">Numer dokumentu <span style={{ color: "var(--mut)" }}>(opcjonalnie)</span>
        <input value={documentNumber} onChange={e => setDocumentNumber(e.target.value)} className="mt-1 w-full rounded-lg px-3 py-2 text-sm" style={{ background: "var(--bg)", border: "1px solid var(--line)" }} placeholder="np. FV/09/2026/123" />
      </label>
    ...
    <div className="mt-4 space-y-2">
      ...
      {!loading && documents.length === 0 && <div className="text-xs" style={{ color: "var(--mut)" }}>{invoiceRequested ? "Dokument nie został jeszcze dodany." : "Brak dokumentów przy tym zamówieniu."}</div>}
    ...
    {mode === "seller" && <div className="mt-3 text-[11px]" style={{ color: "var(--mut)" }}>W przyszłości dokumenty z połączonego Sunrise Studio będą pojawiały się w tej samej sekcji automatycznie.</div>}
  </section>;
}
