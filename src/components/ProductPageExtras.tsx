...
export default function ProductPageExtras({offerId,verifyKind}:{offerId:string;verifyKind:"vehicle"|"property"|null}){
 ...
 useEffect(()=>{let alive=true;getOffer(offerId).then(async o=>{if(!alive)return;setOffer(o);if(o?.seller_id){const {data}=await supabase.schema("market").rpc("seller_public_reputation",{p_seller:o.seller_id});if(alive&&data)setRep(data as Reputation);const {data:prof}=await supabase.schema("market").rpc("seller_public_profile",{p_seller:o.seller_id});if(alive&&prof)setSellerOffers(((prof as any).offers||[]).filter((x:any)=>x.id!==offerId).slice(0,8));}try{const c=await bookingPublicCatalogV2(offerId);if(alive&&c){setBookable(true);setBookingType(c.config.booking_type)}}catch{/* optional */}}).catch(()=>{});return()=>{alive=false}},[offerId]);
 ...
 return <section className="mx-auto max-w-7xl px-4 pb-4">
    ...
    {sellerOffers.length>0&&<div className="mt-6"><div className="flex items-end justify-between gap-3"><h2 className="text-xl font-semibold">Inne od tego sprzedawcy</h2>{rep&&<Link to={`/sprzedawcy/${rep.seller_id}`} className="shrink-0 whitespace-nowrap text-xs font-semibold" style={{color:"var(--gold)"}}>Wszystkie →</Link>}</div>
      <div className="mt-3 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 [scrollbar-width:none]">{sellerOffers.map((x:any)=><a key={x.id} href={`/produkt/${x.id}`} className="w-[46%] shrink-0 snap-start overflow-hidden rounded-2xl sm:w-[220px]" style={{background:"var(--glass)",border:"1px solid var(--line)"}}><div className="aspect-[4/3] overflow-hidden" style={{background:"var(--header)"}}>{x.image_url?<img src={x.image_url} alt="" loading="lazy" className="h-full w-full object-cover"/>:<div className="grid h-full place-items-center text-3xl">🌅</div>}</div><div className="p-3"><div className="font-bold" style={{color:"var(--gold)"}}>{money(x.price_gross)}</div><div className="mt-0.5 line-clamp-2 text-sm font-semibold leading-5">{x.title}</div></div></a>)}</div></div>}
