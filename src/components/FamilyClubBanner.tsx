export default function FamilyClubBanner({ slim = false }: { slim?: boolean }) {
  return (
    <a href="/konto" className="block rounded-2xl overflow-hidden"
       style={{ background: "linear-gradient(120deg, rgba(232,137,26,.22), rgba(232,137,26,.04) 60%), linear-gradient(135deg,#16223C,#101012)", border: "1px solid rgba(232,137,26,.45)", boxShadow: "0 14px 38px -20px rgba(0,0,0,.6)" }}>
      <div className={`flex items-center gap-4 ${slim ? "px-4 py-3" : "px-5 py-4"}`}>
        <div className="shrink-0 grid place-items-center rounded-xl"
             style={{ width: slim ? 40 : 54, height: slim ? 40 : 54, background: "linear-gradient(135deg,#F5A623,#E8891A)", color: "#241606", fontSize: slim ? 22 : 30 }}>☀</div>
        <div className="flex-1 min-w-0">
          <div className="font-semibold tracking-wide" style={{ color: "#F5A623", fontSize: slim ? 14 : 16 }}>SUNRISE FAMILY CLUB</div>
        ...
        <span className={`shrink-0 font-semibold rounded-xl ${slim ? "text-xs px-3 py-1.5" : "text-sm px-4 py-2"} hidden sm:block`}
              style={{ background: "linear-gradient(135deg,#F5A623,#E8891A)", color: "#241606" }}>Dołącz →</span>
      </div>
    </a>
  );
}
